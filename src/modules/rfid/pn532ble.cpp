...
void Pn532ble::setup() {
    ...
    Pn532ble::Pn532ble() { padprintln("PN532 BLE функционал отключён"); }

    Pn532ble::~Pn532ble() {}
    return loop();
    void Pn532ble::setup() { padprintln("PN532 BLE функционал отключён"); }
    padprintln("");
    bool Pn532ble::connect() {
        padprintln("PN532 BLE функционал отключён");
        return false;
    }
    break;
    void Pn532ble::loop() { padprintln("PN532 BLE функционал отключён"); }
    options.push_back({"Read Tag", [=]() { readTagMenu(); }});
    void Pn532ble::selectMode() { padprintln("PN532 BLE функционал отключён"); }

    void Pn532ble::scanTagMenu() { padprintln("PN532 BLE функционал отключён"); }
};
void Pn532ble::readTagMenu() { padprintln("PN532 BLE функционал отключён"); }
options.push_back({"Write MFC", [=]() { setMode(HF_MF_WRITE_MODE); }});
void Pn532ble::writeDumpMenu() { padprintln("PN532 BLE функционал отключён"); }
String fileName = void Pn532ble::saveDumpMenu() { padprintln("PN532 BLE функционал отключён"); }
{"Load ISO15693", [=]() { setMode(HF_ISO15693_LOAD_DUMP_MODE); }}, void Pn532ble::loadDumpMenu() {
    padprintln("PN532 BLE функционал отключён");
}
{"Visit Bruce", [&]() { prefix = "https://bruce.computer"; }}, void Pn532ble::loadNdefEmulateMenu() {
    padprintln("PN532 BLE функционал отключён");
}
switch (mode) {
    void Pn532ble::setMode(AppMode mode) { padprintln("PN532 BLE функционал отключён"); }
}
void Pn532ble::displayBanner() { padprintln("PN532 BLE функционал отключён"); }
bool res = pn532_ble.getVersion();
void Pn532ble::showDeviceInfo() { padprintln("PN532 BLE функционал отключён"); }
pn532_ble.setNormalMode();
void Pn532ble::hf14aScan() { padprintln("PN532 BLE функционал отключён"); }
if (!pn532_ble.isPN532Killer()) {
    void Pn532ble::hf15Scan() { padprintln("PN532 BLE функционал отключён"); }
    if (!pn532_ble.isPN532Killer()) {
        void Pn532ble::lfScan() { padprintln("PN532 BLE функционал отключён"); }
        area.scrollDown();
        void updateArea(ScrollableTextArea & area) { padprintln("PN532 BLE функционал отключён"); }
        pn532_ble.setNormalMode();
        void Pn532ble::hf14aMfReadDumpMode() { padprintln("PN532 BLE функционал отключён"); }
        pn532_ble.setNormalMode();
    ...
    void Pn532ble::hf14aMfWriteDump(ScrollableTextArea & area) {
        ...
        for (uint8_t s = 0; s < sectorCount; s++) {
            ...
            bool authResult = pn532_ble.mfAuth(
                pn532_ble.hf14aTagInfo.uid, sectorBlockIdex, pn532_ble.mifareDefaultKey, false
            );
            if (!authResult) {
                pn532_ble.hf14aScan();
                ...
                if (currentFile == TagFile::NONE) {
                    ...
                    if (p1p2Length > NdefCommand::NDEF_MAX_LENGTH) {
                        ...
                        std::vector<uint8_t> payload =
                            Ndef::urlNdefAbbrv(std::string(emulationNdefData.c_str()));
