...
namespace {
const int MENU_APPLICATION_INDEX = 1;
}
...
void ApplicationManager::update() {
    ...
    if (currentApplicationIndex >= 0 && applications[currentApplicationIndex] != nullptr) {
        ...
        // Regular applications signal exit by clearing their running flag.
        if (!applications[currentApplicationIndex]->isRunning() &&
            currentApplicationIndex != MENU_APPLICATION_INDEX &&
            applicationCount > MENU_APPLICATION_INDEX &&
            applications[MENU_APPLICATION_INDEX] != nullptr) {
            switchToApplication(MENU_APPLICATION_INDEX);
        }
    }
}
