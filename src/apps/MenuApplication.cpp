...
void MenuApplication::render(Display* display) {
    ...
    display->drawString(2, 2, "HOME");
    
    // Draw app count instead of placeholder time
    display->setTextColor(0, 1);
    display->setTextSize(1);
    if (appManager != nullptr) {
        String countLabel = String(appManager->getApplicationCount() - 1) + " apps";
        display->drawString(88, 2, countLabel);
    }
    ...
    if (appManager != nullptr) {
        ...
        for (int i = 0; i < visibleApps && (startIndex + i) < appCount; i++) {
            ...
            // Icon mapping
            if (strcmp(appName, "Clock") == 0) {
                display->drawBitmap(iconX, iconY - 1, stopwatchIcon, 8, 8, isSelected ? 0 : 1);
            } else if (strcmp(appName, "Timer") == 0) {
                display->drawBitmap(iconX, iconY - 1, stopwatchIcon, 8, 8, isSelected ? 0 : 1);
            } else if (strcmp(appName, "Tally Counter") == 0) {
                display->drawBitmap(iconX, iconY - 1, calculatorIcon, 8, 8, isSelected ? 0 : 1);
            } else if (strcmp(appName, "Settings") == 0) {
                display->drawBitmap(iconX, iconY - 1, settingsIcon, 8, 8, isSelected ? 0 : 1);
            } else if (strcmp(appName, "Device Info") == 0) {
                display->drawBitmap(iconX, iconY - 1, infoIcon, 8, 8, isSelected ? 0 : 1);
            } else {
                display->drawBitmap(iconX, iconY - 1, homeIcon, 8, 8, isSelected ? 0 : 1);
            }
    ...
    display->setTextSize(1);
    display->setTextColor(1, 0);
    display->drawString(4, 56, "OK open");
    display->drawString(78, 56, "UP/DN nav");
